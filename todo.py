...
args = sys.argv
args_len = len(args) - 1
notefile = "/home/wolfy/.cache/todo/notes"
groupfile = "/home/wolfy/.cache/todo/groups"
...
def list():
	...
	counter_alt = 0
	for a in ls_array:
		print(colored(alphabet[counter_alt] + ")", "green") + colored(a[0], "blue") + ":")
		counter = 1
		for b in a[1]:
			print("      " + str(counter) + ". " + b)
			counter+=1
	...
	for line in note_lines:
		...
		if(line_group == group):
			...
			if(note_nr != str(counter)):
				...
				if(args_len == 4):
					file.write(args[3] + " " + line_state + " " + args[4] + "\n")
				else:
